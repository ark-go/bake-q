...
import (
	...
	"github.com/ark-go/bake-go/src/postgres/tables_query/products"
	"github.com/ark-go/bake-go/src/postgres/tables_query/productvid"
)
...
func (t *Tab_products) CreateQuery() (*iface.ReturnQueryT, error) {
	...
	case `update`:
		...
		if err != nil {
			...
			pr := &products.Load{Session: t.Session, InParams: t.InParams}
			if err := t.ReturningQueryToMap(result, &pr.RowReturning); err != nil {
				return nil, err
			} // поместим результат в структуру, для передачи предыдущего запроса точнее ID
			otv := pr.MakeQuery(t.InParams) // подготовка запроса, проверка параметров уточнение
	...
	case `add`:
		...
		if err != nil {
			...
			pr := &products.Load{Session: t.Session, InParams: t.InParams}
			if err := t.ReturningQueryToMap(result, &pr.RowReturning); err != nil { // поместим результат в структуру, для передачи предыдущего запроса точнее ID
				return nil, err
			}
			otv := pr.MakeQuery(t.InParams) // подготовка запроса, проверка параметров уточнение
			return otv, nil
