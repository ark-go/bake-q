...
import (
	...
	_ "github.com/ark-go/bake-go/src/utils"
...
func (s *RedisStore) Get(r *http.Request, name string) (*SessionJson, error) {
	...
	if err != nil {
		...
		if code == "BadSigned" {
			...
			err = s.delete(r.Context(), session)
			return nil, err
		}
		if code == "BadCode" {
			// возможно код не наш
...
func (s *RedisStore) Save(r *http.Request, w http.ResponseWriter) error {
	...
	session, ok := r.Context().Value(SessionKey("sessKey")).(*SessionJson)
	if !ok {
		log.Println("сессия не обнаружена")
	}
